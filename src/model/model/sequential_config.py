...
class DenseLayerConfig(BaseModel):
    type: Literal["dense"]
    units: int = Field(..., gt=0, description="Number of units in the dense layer.")
    activation: Literal["lrelu", "relu", "tanh", "sigmoid", "softmax"]
    kernel_initializer: Literal["he_normal", "glorot_uniform"] = "glorot_uniform"
    kernel_regularizer: Optional[Literal["l2", "l1"]] = None

class DropoutLayerConfig(BaseModel):
    type: Literal["dropout"]
    rate: float = Field(..., ge=0.0, le=1.0, description="Dropout rate (between 0 and 1).")
...
class OptimizerConfig(BaseModel):
    ...
    learning_rate: float = Field(..., gt=0, description="Learning rate must be positive.")
...
class SequentialModelConfig(BaseModel):
    ...
    loss: Literal["binary_crossentropy", "categorical_crossentropy", "mean_squared_error"]
    batch_size: int = Field(..., gt=0, description="Batch size must be greater than 0.")
    epochs: int = Field(..., gt=0, description="Number of epochs must be greater than 0.")

    @model_validator(mode="after")
    def validate_layer_order(cls, values) -> Any:
        """
        Validate that the first layer is an input layer and ensure consistent layer ordering.
        """
        ...
        for i in range(1, len(values.layers)):
            ...
            if values.layers[i-1].type == "dropout" and values.layers[i].type == "dropout":
